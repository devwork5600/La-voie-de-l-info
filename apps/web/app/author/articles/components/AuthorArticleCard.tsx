...
export interface AuthorArticleCardData {
  id: string;
  title: string;
...
const AuthorArticleCard: React.FC<{ article: AuthorArticleCardData }> = ({
  article,
}) => {
  ...
  const { mutate: removeArticle, isPending } = useMutation({
    ...
    onSuccess: (result) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["authorStats"] });
    },
    onError: () => {
